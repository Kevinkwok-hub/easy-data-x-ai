...
::: tip 配套实战
这篇负责定义 RAG 的产品问题、六段链路和验收口径。完成后建议继续学习 [D2：AI 应用的数据层](../dev/D2%20课程稿：AI%20应用的数据层.md) 与 [D3：Agentic RAG 实战](../dev/D3%20课程稿：Agentic%20RAG%20实战.md)。D3 已把查询分析、混合检索、权限过滤、融合重排、引用校验、失败重试和 60 条离线评测落成可运行代码。
:::
...
如果你准备把问题推进到实现阶段，可以直接使用下面这张交接表。它让 P2 的判断在 D2、D3 中有明确落点。

| 你在 P2 定位到的问题 | 对应实战 | 最小验收证据 |
| --- | --- | --- |
| 文档切分或元数据不完整 | D2 数据准备 | 固定案例能召回完整上下文，版本、来源、权限字段可过滤 |
| 精确标识符召回错误 | D2 / D3 混合检索 | 错误码、版本号、季度、函数名进入 Top-3 |
| 多路结果重复或冲突 | D3 融合重排 | 文档 ID 去重，权限先过滤，时效和来源规则可解释 |
| 回答脱离检索证据 | D3 引用校验 | 每个引用都来自本次上下文，未知引用触发重试或拒答 |
| 不知道优化是否有效 | D3 离线评测 | 固定回归集输出 Hit@1、Hit@3、MRR、召回率、拒答与调用量 |

> 这张表不是要求 PM 写代码，而是帮助产品和工程对同一个“完成标准”达成一致。具体运行命令和报告示例见 D3 第五部分。
